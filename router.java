...
public class router {
	...
	public static class transmission implements Callable{
		...
		public Long call() throws Exception{
			//Bind the IP address of client and server
			InetAddress serverAddress = InetAddress.getByName("192.168.85.4");
			InetAddress clientAddress = InetAddress.getByName("192.168.85.2");
			DatagramSocket socket2 = new DatagramSocket();
			DatagramSocket socket3 = new DatagramSocket();
			byte[] byteBuffer = gstr((int)length).getBytes();
			DatagramPacket sendPacket = new DatagramPacket(byteBuffer, byteBuffer.length,clientAddress,4701);
			DatagramPacket sendPacket2 = new DatagramPacket(byteBuffer, byteBuffer.length,serverAddress,4701);
			...
			//Sending information of Rin2 to client
			if(flag == 1 && sequence == 1){
				while(true){
					...
					DatagramPacket ctnlPacket = new DatagramPacket(rslt.getBytes(),rslt.getBytes().length,magAddress,4704);
			...
			//For applications placed at client
			//Sending information of Rs to client
			if(flag == 1 && sequence == 2){
				while(true){
					...
					//Reporting result to management server
					rslt = Long.toString(result);
					DatagramPacket ctnlPacket = new DatagramPacket(rslt.getBytes(),rslt.getBytes().length,magAddress,4705);
					socket.send(ctnlPacket);
			...
			if(flag == 2){
				while(true){
					...
					rslt = Long.toString(result);
					DatagramPacket ctnlPacket = new DatagramPacket(rslt.getBytes(),rslt.getBytes().length,magAddress,4704);
					DatagramPacket ctnlPacket2 = new DatagramPacket("0".getBytes(),"0".getBytes().length,magAddress,4705);
					socket.send(ctnlPacket);
			...
			if(flag == 3 && sequence == 1){
				while(true){
					while(num < 101){
						if (num == 1){
							sendtime[0] = System.nanoTime();
						}
						socket3.send(sendPacket2);
						if (num == 100){
							sendtime[1] = System.nanoTime();
						}
						num += 1;
	...
	public static void main(String[] args) throws Exception {
		...
		InetAddress magAddress = InetAddress.getByName("192.168.85.1");
		DatagramSocket socket = new DatagramSocket();
		DatagramSocket rsocket = new DatagramSocket(4700);
		DatagramPacket packet = new DatagramPacket(new byte[100],100);
		
		//Variable array for storing applications
		ArrayList total_app = new ArrayList();
		
		//Generate applications at router
		Application c = new Application(2000, 3000, 2000, 0);
		total_app.add(c);
		Application d = new Application(2000, 3000, 2000, 4000);
		total_app.add(d);
		
		//Sending total number of applications to management server
		String tmpstr = Integer.toString(total_app.size());
		DatagramPacket ctnlPacket = new DatagramPacket(tmpstr.getBytes(),tmpstr.getBytes().length,magAddress,4701);
		socket.send(ctnlPacket);
		...
		for(int i=0;i<total_app.size();i++){
			...
			ctnlPacket = new DatagramPacket(tmp.getBytes(),tmp.getBytes().length,magAddress,4701);
			socket.send(ctnlPacket);
		}
