...
public class outrouter {
	...
	public static class transmission implements Callable{
		...
		public Long call() throws Exception{
			//Bind the IP address of server
			InetAddress serverAddress = InetAddress.getByName("192.168.85.4");
			DatagramSocket socket2 = new DatagramSocket();
			byte[] byteBuffer = gstr((int)length).getBytes();
			DatagramPacket sendPacket = new DatagramPacket(byteBuffer, byteBuffer.length,serverAddress,4701);
			...
			while(true){
				...
				DatagramPacket ctnlPacket = new DatagramPacket(rslt.getBytes(),rslt.getBytes().length,magAddress,4708);
	...
	public static void main(String[] args) throws Exception {
		...
		InetAddress magAddress = InetAddress.getByName("192.168.85.1");
		DatagramSocket socket = new DatagramSocket();
		DatagramSocket orsocket = new DatagramSocket(4700);
		DatagramPacket packet = new DatagramPacket(new byte[100],100);
		
		for(;;){
			orsocket.receive(packet);
